...
class UnitTest(unittest.TestCase):
  ...
  def testAnnotationExtraction(self):
    ...
    self.assertEqual(self.ingest._ACEIngest__extractAnnotations("as*aa I said"), ("aa I said", [Annotation("correction", "as", 2, 2)]))
    self.assertEqual(self.ingest._ACEIngest__extractAnnotations("b as*aa I said"), ("b aa I said", [Annotation("correction", "as", 4, 4)]))
    self.assertEqual(self.ingest._ACEIngest__extractAnnotations("hello as*aa I said"), ("hello aa I said", [Annotation("correction", "as", 8, 8)]))
    self.assertEqual(self.ingest._ACEIngest__extractAnnotations("hello this*bb is the thing*thng, we all need*ned to do right*rgt"), \
      ("hello bb is the thng, we all ned to do rgt", [Annotation("correction", "this", 8, 8), Annotation("correction", "thing", 20, 20),                                                                                                                                         Annotation("correction", "need", 32, 32), Annotation("correction", "right", 42, 42)]))


  def testTransposeCorrections(self):
    self.assertEqual(self.ingest._ACEIngest__transposeCorrections([AnnotatedText("one", [Annotation("a", "b", 0,0)]), AnnotatedText("two", [Annotation("c","d",1,1)])]), \
                                          [AnnotatedText("one", [Annotation("a", "b", 0,0)]), AnnotatedText("two", [Annotation("c","d",1,1)])] , \
                    )
    self.assertEqual(self.ingest._ACEIngest__transposeCorrections([AnnotatedText("one", [Annotation("a", "b", 0,0)])]), \
  ...
  def testHtmlParseOnSampleA01(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('A01a'):
        ...
        self.assertEqual(parser.meta_data[master_key]['Publisher'], u"Samples 'Used by permission of News Limited")
        ...
        self.assertEqual(parser.meta_data[master_key]['Title'], u"Gala opening for extension to Qld Govt's DP centre")
  ...
  def testHtmlParseOnSampleA22f(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('A22f'):
        self.assertEqual(parser.meta_data[master_key]['Source'], u'The Courier Mail 4 December 1986 (2013 words)')
        self.assertEqual(parser.meta_data[master_key]['Publisher'], u"Samples 'Used by Permission of Queensland Newspapers Pty Ltd")
  ...
  def testHtmlParseOnSampleA23b(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('A23b'):
        self.assertEqual(parser.meta_data[master_key]['Source'], u'The Courier Mail 15 October 1986 (2001 words)')
        self.assertEqual(parser.meta_data[master_key]['Publisher'], u"Samples 'Used by Permission of Queensland Newspapers Pty Ltd")
        ...
        self.assertEqual(parser.meta_data[master_key]['Title'], u'Myth and fable alive and well on the Australian wine scene')
  ...
  def testHtmlParseOnSampleA38(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('A38c'):
        ...
        self.assertEqual(parser.meta_data[master_key]['Author'], u'Kevin Norbury, Megan Jones and Ross Brundrett')
  ...
  def testHtmlParseOnSampleA44(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('A44b'):
        ...
        self.assertEqual(parser.meta_data[master_key]['Title'], u'An eight year battle - then AB licence is granted')
  ...
  def testHtmlParseOnSampleB01a(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('B01a'):
        ...
        self.assertEqual(parser.meta_data[master_key]['Title'], u"Letters: L Gild, Yokine WA; U Bell, Mundaring WA; M Fischer, Perth WA; CW Allen, Cremorne NSW; Di Yerbury, North Sydney NSW; M Hill, Lakemba NSW; R Aschenbrenner, Ferntree Gully Vic; N Manette, Lane Cove NSW; JK McWilliam, Bardon Qld")
        self.assertEqual(parser.meta_data[master_key]['Publication Date'], u'28 October 1986')
  ...
  def testHtmlParseOnSampleB01b(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('B01b'):
        self.assertEqual(parser.meta_data[master_key]['Source'], u'The Weekend Australian 1-2 November 1986 (2007 words)')
        self.assertEqual(parser.meta_data[master_key]['Publisher'], u"Samples used by Permission of News Limited")
        self.assertEqual(parser.meta_data[master_key]['Word Count'], u'2007 words')
        self.assertEqual(parser.meta_data[master_key]['Title'], u"Letters:T Freeman, Croydon Vic; GR Ryan, Mt Pleasant; PR Zeeman, Launceston; P Bonar, Findon SA; IG Hope, Kelso NSW; N Nichols, Buderim Qld; IP Youles, Middleton SA; M DeChadbannes, Beaudesert; RF Harrison, Cambah Act; TV Davies, Pymble NSW")
  ...
  def testHtmlParseOnSampleB13b(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('B13b'):
        ...
        self.assertEqual(parser.meta_data[master_key]['Title'], u"Letters: CS Jenkins, Aspley; A Conway, Gold Coast; JW Mooney, Hendra; R Franklin, Kenmore; S Ryan, Minister for Education, Canberra; L Brownsey, Banyo; Dr S Woodford, Mt Isa Base Hospital; ED Halburn, Algester")
  ...
  def testHtmlParseOnSampleB03c(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('B03c'):
        self.assertEqual(parser.meta_data[master_key]['Source'], u'The Daily Mirror 29 September 1986 (2010 words)')
        self.assertEqual(parser.meta_data[master_key]['Publisher'], u"Samples Used by Permission of News Limited")
  ...
  def testHtmlParseOnSampleK01(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('K01'):
        ...
        self.assertEqual(parser.meta_data[master_key]['Publisher'], u"Sample Used by Permission of Fremantle Arts Centre Press")
  ...
  def testHtmlParseOnSampleKL07(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('L07'):
        ...
        self.assertEqual(parser.meta_data[master_key]['Publisher'], u"Sample Used by Permission of Rastor Pty Ltd")
  ...
  def testHtmlParseOnSampleR01(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('R01'):
        ...
        self.assertEqual(parser.meta_data[master_key]['Publisher'], u"Sample Used by Permission of Watermark Press Pty Ltd")
        self.assertEqual(parser.meta_data[master_key]['Word Count'], u'2001 words')
        self.assertEqual(parser.meta_data[master_key]['Title'], u"30:Tax Avoidance and a Big Mac; 31:The Speech Writer; 32:Death At The Cross")
  ...
  def testHtmlParseOnSampleR05(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('R05'):
        self.assertEqual(parser.meta_data[master_key]['Source'], u'Growing Up Catholic: An infinitely funny guide for the faithful, the fallen and everyone in between 1986     (2007 words)')
        ...
        self.assertEqual(parser.meta_data[master_key]['Author'], u'Gabrielle Lord, Mary Jane Frances, Carolina Mears, Jeffrey Allen, Joseph Stone, Maureen Anne Teresa Kelly, Richard Glen, Michael Door')
  ...
  def testHtmlParseOnSampleR14f(self):
    ...
    for master_key in sorted(parser.meta_data.iterkeys()):
      if master_key.startswith('R14f'):
        ...
        self.assertEqual(parser.meta_data[master_key]['Publisher'], u"Samples Used by Permission of Neil Hulm Publishing")
