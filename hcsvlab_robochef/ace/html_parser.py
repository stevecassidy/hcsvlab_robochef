...
class AceHTMLParser(HTMLParser):
        ...
        if self.nth_row > 1 and self.val_present and not re.search('^\s+$', data):
            ...
            # Processing of title, source or publication
                ...
                if 'Author' in self.meta_data[self.current_sample_id]:
                    self.meta_data[self.current_sample_id][u'Author']\
                        = self.meta_data[self.current_sample_id][u'Author'].strip() + ' and ' + data.strip()
                else:
    ...
    def __add_sourceorpublication__(self, sample_id, data):
        ...
        if re.search('\(\d+\swords\)$', data.strip()):
            ...
            pub_date = self.extract_date(
                self.current_source)  # Store the publication date as it might be useful for later
            if len(pub_date) > 0:
            ...
            if len(pub_date) > 0:
                ...
                for val in self.group_ids:
                    ...
                    if 'Publisher' in self.meta_data[val]:
                        self.meta_data[val][u'Publisher']\
                            = self.meta_data[val][u'Publisher'].strip() + ' & ' + data.strip()
                    else:
    ...
    def __add_sample__(self, sample_id):
        """
        Adds the sample id to the group id list, if however the sample signifies the start of a new group,
        we reset the list
        """
