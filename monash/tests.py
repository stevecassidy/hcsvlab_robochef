...
from hcsvlab_robochef.rdf.map import Literal
...
class UnitTest(AusNCTest):
    ...
    corpus_basedir = configmanager.get_config("CORPUS_BASEDIR", "../input/") \
                     + 'Monash Corpus of Australian English/Transcripts-sanitised/'
    ...
    def testMEBH1M1(self):
        filename = self.corpus_basedir + 'MEBH1M1_Sanitised.doc'
        ...
        (meta_graph, ann_graph) = ingest._MonashIngest__serialise(
            self.corpus_basedir, '/tmp', filename, sampleid, 'MEBH1M1_Sanitised.doc', raw, text, metadata, anns)
        ...
        self.one_of(meta_graph, itemuri, AUSNC.MaternalGrandmothersEthnicity, Literal(
            u'(Mentions a grandmother who died in Australia, but also others who are living in England and Scotland)'))
        self.one_of(meta_graph, itemuri, AUSNC.LanguagesSpokenathome, Literal(u'English'))
        self.one_of(meta_graph, itemuri, AUSNC.notes, Literal(
            u'Doesn\u2019t know where his parents were born - says that one was born in\nGermany and one in England, or maybe both in England. Mentions grandparents\nin England and Scotland'))
    ...
    def testMEP6(self):
        filename = self.corpus_basedir + 'MEP6_Sanitised.doc'

        ingest = MonashIngest()
        (sampleid, raw, text, metadata, anns) = ingest.ingestDocument(filename)

        (meta_graph, ann_graph) = ingest._MonashIngest__serialise(
            self.corpus_basedir, '/tmp', filename, sampleid, 'MEP6_Sanitised.doc', raw, text, metadata, anns)
    ...
    def testParticipantDescriptions(self):
        ...
        filename = self.corpus_basedir + 'MECG1MA_Sanitised.doc'
        ingest = MonashIngest()
        (sampleid, raw, text, metadata, anns) = ingest.ingestDocument(filename)
        ...
        self.failUnless(any([s == primary for s in spkrs]),
                        "Primary speaker (%s) not in speakers list (%s)" % (str(primary), str(spkrs)))
