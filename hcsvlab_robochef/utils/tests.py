...
class UnitTest(unittest.TestCase):
    ...
    def testFileHandlerOnDirectoryWithDataAndValidExtension(self):
        resultSet = self.fileHandler.getFiles(self.corpus_basedir + 'ace', inclusionPredicate=r'^ace_.\.txt')
        self.assertEqual(len(resultSet), 17)


    def testFileHandlerOnDirectoryWithDataAndInvalidExtension(self):
        resultSet = self.fileHandler.getFiles(self.corpus_basedir + 'ace', inclusionPredicate=r'\w+.rdf')
    ...
    def testSizeOfDirectory(self):
        self.assertEqual(self.fileHandler.getFileSize(self.corpus_basedir), 0)


    def testSizeOfKnownFile(self):
        self.assertEqual(self.fileHandler.getFileSize(self.corpus_basedir + 'ace/ace_a.txt'), 579959)


    def testSizeOfAudioFile(self):
        self.assertEqual(self.fileHandler.getFileSize(self.corpus_basedir + 'md/S1219s1.wav'), 135752)


    def testSizeOfSourceString(self):
        stats = Statistics()
        lines = open(self.corpus_basedir + 'ace/ace_a.txt', 'r').read()
        meta_dict = {}
        stats.append_item_stats(lines, meta_dict)
        self.assertEqual(self.fileHandler.getFileSize(self.corpus_basedir + 'ace/ace_a.txt'), meta_dict['filesize'])
    ...
    def testWordCountOnAudioFile(self):
        stats = Statistics()
        self.assertEqual(stats.get_word_count(self.corpus_basedir + 'md/S1219s1.wav'), 0)


    def testWordCountOnTxtFile(self):
        stats = Statistics()
        self.assertEqual(stats.get_word_count(self.corpus_basedir + 'ace/ace_a.txt'), 92466)
    ...
    def testToFindExistingCompatriotFile(self):
        compatriot = self.fileHandler.findCompatriot(self.corpus_basedir + 'griffith', 'GCSAusE06.doc', '^[\w\d-]+.mp3',
                                                     '^[\w\d-]+.doc')
        self.assertEqual(self.corpus_basedir + 'griffith/aa323c84-7483-10a3-d8b6-603a7f5d852a_1/GCSAusE06.mp3',
                         compatriot)


    def testToFindNonExistingCompatriotFile(self):
        compatriot = self.fileHandler.findCompatriot(self.corpus_basedir + 'griffith', 'GCSAusE99.doc', '^[\w\d-]+.mp3',
                                                     '^[\w\d-]+.doc')
    ...
    def testGenerateOfMetaDict(self):
        ...
        self.assertEqual(res, {'documenttitle': 'GCSAusE01#Text', 'filetype': 'Text', 'filename': 'GCSAusE01-plain.txt',
                               'id': 'GCSAusE01#Text', 'filesize': 22})
