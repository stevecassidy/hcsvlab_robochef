...
from hcsvlab_robochef.rdf.map import FOAF, DBPEDIA, SCHEMA
...
class UnitTest(unittest.TestCase):
    ...
    def with_and_without(self, tst):
        ...
        tst(self)
        self.ingest.setMetaData(self.corpus_basedir + "COOEE/COOEE.XLS")
        tst(self)


    def test25(self):
        filename = self.corpus_basedir + "COOEE/data/1-025.txt"
        ...
        (meta_graph, ann_graph) = self.ingest._CooeeIngest__serialise('/tmp', sampleid, raw, text, metadata, anns,
                                                                      filename)
    ...
    def test244(self):
        filename = self.corpus_basedir + "COOEE/data/1-244.txt"
