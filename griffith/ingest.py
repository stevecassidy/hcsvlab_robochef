...
class GriffithIngest(IngestBase):
    ...
    for fl in rst:
      ...
      for tag in xml_tree.iter():
        if (len(list(tag)) == 0):
          ...
          tags[tag.tag.decode('utf-8')] = u"" if (tmp_text == None) else tmp_text.decode('utf-8')
  ...
  def ingestDocument(self, f):
    ...
    text = subprocess.check_output(["antiword", "-f",f])
  ...
  def parseTopTable(self, string):
    """ Parses the header block after it has been converted to text """
    
    whole = Suppress(u"Transcript Coversheet") \
          + Suppress(u"|") + Suppress(u"|Data") + Suppress(u"|") \
          + utils.tableParser(2).setResultsName("topTable") \

    return whole.parseString(self.__getHeader(string))


  def parseBody(self, string):
    """ Parses the remainder of the document, after it has been converted to text """

    data = string.replace(self.__getHeader(string), "")

    whole = ZeroOrMore(Suppress("\n") ^ Suppress(Literal("((") + CharsNotIn(")") + Literal("))"))) \
        + (OneOrMore(self.__possibleLine()).leaveWhitespace().setParseAction(lambda s, loc, toks: "\n".join(toks))).setResultsName("body") \
        + StringEnd()
  ...
  def __serialise(self, srcdir, outdir, source_file, name, rawtext, body, meta, anns):
    ...
    if compatriot is not None:
      original_doc = { 'filetype': 'Text', 'sampleid': meta['sampleid'], 'rawtext': rawtext, 'text': body, 'sourcepath': source_file}        
      compatriot = { 'filetype': 'Audio', 'sampleid': meta['sampleid'], 'sourcepath': compatriot}
      return serialiser.serialise_multiple(meta['sampleid'], (original_doc, compatriot, ), 'griffith', griffMap, meta, anns)
      
    else:
      return serialiser.serialise_single(meta['sampleid'], 'griffith', rawtext, body, griffMap, meta, anns, source_file)



  def __getHeader(self, string):
    ''' 
    ...
    '''
    return SkipTo(Word("ABCDEFGHIJKLMNOPQRSTUVWXY") + Word(":")).parseString(string)[0]
  
  
  def __markupParser(self):
    markups = Forward()
    markups << oneOrMoreParsers( slurpParser(u"():?,."+unichr(9702)+u"\u00B0\xbf\xba<>=\u2020"+unichr(8593) + unichr(8595))  \
                               ^ micropauseParser()  \
                               ^ pauseParser()       \
                               ^ elongationParser()  \
                               ^ uncertaintyParser() \
                               ...
                               ^ speedParser(markups)       \
                               ^ latchedParser()     \
                               ^ (Suppress(u'\u2020') + markups + Suppress(u'\u2020')).setParseAction(lambda s, loc, toks: toks[0].add_anns_chain([Annotation("unknown", "pic", 0, len(toks[0].text))]))  \
                               )
    return markups
  ...
  def __extractAnnotations(self, data):
    ...
    try:
      ...
      full_anns = ( Optional(pauseParser(), AnnotatedText("", [])) \
                  + self.__ourSpeakerParser()                         \
