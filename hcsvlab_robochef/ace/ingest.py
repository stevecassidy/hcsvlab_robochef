...
class ACEIngest(IngestBase):
    ...
    def setMetaData(self, srcdir):
        ...
        rs = fileHandler.getFiles(srcdir, inclusionPredicate='^KAT\w+\.HTM$',
                                  exclusionPredicate='^KATLS.HTM$|^.DS_Store$')
    ...
    def __ingestSample(self, sample):
        ...
        # Fetch the sample id and source
            sample_id = self.__get_element_content(sample, 'id')
            genre = sample_id[0]
            source = self.__get_element_content(sample, 'source')
            if source == "":
                source = "Unknown"

            self.status = "id: %s source: %s, genre: %s" % (sample_id, source, genre)

            content = []
            for node in sample.childNodes:
                h = open("/tmp/node" + sample_id, 'a')
                h.write('------')
            ...
            h = open("/tmp/" + sample_id, 'w')
            h.write(content.encode("utf-8"))
            h.close()
            content, anns = self.__extractAnnotations(content)

            res[sample_id] = (
            sample.toprettyxml(), {u'sampleid': sample_id, u'genre': genre, u'source': source}, content, anns)
        return res
